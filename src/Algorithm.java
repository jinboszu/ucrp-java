...
public class Algorithm {
    ...
    private boolean search(int level) {
         ...
         * Prepare Rule 10 (SC)
         *
         * group_src_right[s] = max{k | pk == p[s][h[s]] && sk > s && last_change_type[sk] == MOVE_OUT}
         */
        int min_prio = curr_state.q[curr_state.s_min][curr_state.h[curr_state.s_min]];
        int[] group_src_right = new int[n_stacks];
        int[] group_src_temp_offset = new int[max_prio - min_prio];
        for (int s = n_stacks - 1; s >= 0; s--) {
            group_src_right[s] = curr_state.h[s] == 0 ? 0 : group_src_temp_offset[curr_state.p[s][curr_state.h[s]] - min_prio - 1];
            if (curr_state.last_change_type[s] == Type.MOVE_OUT) {
                int k = curr_state.last_change_time[s];
                int pk = path[k - 1].p;
                if (pk > min_prio) {
                    group_src_temp_offset[pk - min_prio - 1] = Math.max(group_src_temp_offset[pk - min_prio - 1], k);
                }
            }
        }
        ...
        for (int sn = 0; sn < n_stacks; sn++) {
             ...
             * Prepare Rule 11 (SD)
             *
             * group_dst_right[d] = max{k | pk == pn && dk > d && last_change_type[dk] == MOVE_IN}
             */
            int[] group_dst_right = new int[n_stacks];
            ...
            for (int d = n_stacks - 1; d >= 0; d--) {
                ...
                if (curr_state.last_change_type[d] == Type.MOVE_IN) {
                    ...
                    int pk = path[k - 1].p;
                    if (pk == pn) {
                        group_dst_temp = Math.max(group_dst_temp, k);
                    }
                }
            }
            ...
            for (int dn = 0; dn < n_stacks; dn++) {
                 ...
                 */
                if (curr_state.last_change_time[sn] < max_last_move_out_right[sn] && curr_state.last_change_time[dn] < max_last_move_out_right[sn]) {
